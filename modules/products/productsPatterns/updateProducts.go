...
type IUpdateProductProductBuilder interface {
	initTransaction() error
	initQuery()
	updateTitleQuery()
	updateDescripionQuery()
	updatePriceQuery()
	updateCategory() error
	insertImages() error
	getOldImages() []*entities.Image
	deleteOldImage() error
	closeQuery()
	updateProduct() error
...
func UpdateProductBuilder(db *sqlx.DB, req *products.Product, filesUsecase filesusecases.IFilesUsecase) IUpdateProductProductBuilder {
...
type updateProductEngineer struct {
	builder IUpdateProductProductBuilder
}

func UpdateProductEngineer(builder IUpdateProductProductBuilder) *updateProductEngineer {
...
func (b *updateProductBuilder) updateDescripionQuery() {
...
func (b *updateProductBuilder) deleteOldImage() error {
...
func (en *updateProductEngineer) sumQueryFields() {
	...
	en.builder.updateDescripionQuery()
...
func (en *updateProductEngineer) UpdateProduct() error {
	...
	if en.builder.getImagesLen() > 0 {
		if err := en.builder.deleteOldImage(); err != nil {
			return err
		}
