...
type IAppinfoRepository interface {
	...
	DeleteCategory(catagoryId int) error
}
...
func (r *appinfoRepository) FindCategory(req *appinfo.CatagoryFilter) ([]*appinfo.Category, error) {
	query := `
	SELECT
		"id",
		"title"
...
func (r *appinfoRepository) DeleteCategory(catagoryId int) error {
	ctx := context.Background()

	query := `DELETE FROM "categories" WHERE "id" = $1;`

	if _, err := r.db.ExecContext(ctx, query, catagoryId); err != nil {
